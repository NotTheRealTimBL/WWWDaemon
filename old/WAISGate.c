/*	WorldWideWeb - Wide Area Informaion Server Gateway	WAISGate.c
**	==================================================
**
**	This module allows a WWW daemon to read data from a remote  WAIS
	...
	diagnosticRecord **, d)
{
  ...
  for (i = 0; d[i] != NULL; i++) {
    if (d[i]->ADDINFO != NULL) {
#ifdef GEN_HT
      HText_appendText(HT, "Diagnostic code is ");
      HText_appendText(HT, d[i]->DIAG);
      HText_appendCharacter(HT, ' ');
      HText_appendText(HT, d[i]->ADDINFO);
      HText_appendParagraph(HT);
#else
      fprintf(client, "Code: %s, %s\n", d[i]->DIAG, d[i]->ADDINFO);
#endif
    }
  }
}
    ...
    char *,	 		keywords)
{
  ...
  HText_setStyle(HT, glossaryStyle);
#else
  fprintf(client, "%s<dl>\n", line);
#endif 
...
#else
		fprintf(client,
		    "<DT>%s\n<DD>Score: %4ld, lines:%4ld (bad file name)\n",
		    headline,
		    head->Score,
		    head->Lines);
#endif
...
#else
		fprintf(client,
		    "<DT>%s\n<DD>Score: %4ld, lines:%4ld (bad docid)\n",
		    headline,
		    head->Score,
		    head->Lines);
#endif
...
#else
	fprintf(client, "\ncode record %2d, (dont know how to display) ", i);
#endif
...
#ifdef GEN_HT
		HText_appendText(HT, "No text was returned!\n");
#else
